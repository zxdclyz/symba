...
void StockOrderBook::Sort() {
    ...
    reverse(SortedListBids.begin(),
            SortedListBids.begin() + Bids.size()); // reverse() puts it by descending order, good for Bids
        ...
        for (int m = 0; m < int(Bids.size()); m++) {
            if (Bids[m].Price == SortedListBids[k]) {
                ...
                Bids.erase(Bids.begin() +
                           m); // To ensure this order is not taken again in the next count if another bid has the same price
        ...
        for (int m = 0; m < int(Asks.size()); m++) {
            if (Asks[m].Price == SortedListAsks[k]) {
                ...
                Asks.erase(Asks.begin() +
                           m); // To ensure this order is not taken again in the next count if another ask has the same price
...
vector<double>
StockOrderBook::Clear(vector<Agent> &Market, int t, int j, string Plot, const filesystem::path &output_dir) {
    ...
    if (Plot == "On") {
        outputLog << endl << endl << "***************CLEAR() AT TIME t=" << t << "**************" << endl;
    };
    for (int k = -1; k < int(min(Bids.size(), Asks.size())); k++) {
        if (int(min(Bids.size(), Asks.size())) ==
            0) {/*outputLog << "Function Clear() not activated" << endl;*/ break; }
        if (k == -1) { k = 0; };
        ...
        TempClearingPrice =
                (Bids[k].Price + Asks[k].Price) / 2; // Transaction taken at midprice, a neutral and common solution
        bool C1 = Bids[k].Price >= Asks[k].Price; // Bid is larger than Ask at a given level k of the order book
        bool C2 = Market[Asks[k].Agent].Stocks[Asks[k].Stock].StockQuantity >
                  0; // Seller has non-zero quantity of this stock
        bool C3 = Market[Bids[k].Agent].RFA >= TempClearingPrice * TempQuantity; // Buyer has enough RFA to buy
        bool C4 = Market[Asks[k].Agent].Stocks[Asks[k].Stock].StockQuantity >=
                  TempQuantity; // Seller has enough quantities to sell
        if (C1 && C2 && C3 && C4) {
            ...
            // Output
            if (Plot == "On") {
                outputLog << "Level " << LevelCount << ": Agent " << Bids[k].Agent << " with Q="
                          << Market[Bids[k].Agent].Stocks[Bids[k].Stock].StockQuantity - TempQuantity << "->"
                          << Market[Bids[k].Agent].Stocks[Bids[k].Stock].StockQuantity << " longs " << TempQuantity
                          << " of Stock " << Bids[k].Stock << " from Agent " << Asks[k].Agent << " with Q="
                          << Market[Asks[k].Agent].Stocks[Asks[k].Stock].StockQuantity + TempQuantity << "->"
                          << Market[Asks[k].Agent].Stocks[Asks[k].Stock].StockQuantity << " for $" << TempClearingPrice
                          << " (midprice from " << Asks[k].Price << "~" << Bids[k].Price << ")" << endl;
            }; // closes if

            // Proper draft of all orders
            if (Bids[k].Q > TempQuantity) {
                Bids[k].Q -= TempQuantity;
                Asks.erase(Asks.begin() + k);
                k -= 1;
            } // Buyer stays in business if still eager to buy more from next seller
            else if (Asks[k].Q > TempQuantity) {
                Asks[k].Q -= TempQuantity;
                Bids.erase(Bids.begin() + k);
                k -= 1;
            }; // Seller stays in business if still eager to sell more to next buyer
            ...
            if (Plot == "On") {
                outputLog << "Level " << LevelCount << ": not cleared since bid=" << Bids[k].Price << " < ask="
                          << Asks[k].Price << endl;
            };
        }
        else if (!C2) {
            if (Plot == "On") {
                outputLog << "Level " << LevelCount << ": not cleared since seller out of stock" << endl;
            };
        }
        else if (!C3) {
            if (Plot == "On") {
                outputLog << "Level " << LevelCount << ": not cleared since buyer hasn't enough RFA to buy ($"
                          << Market[Bids[k].Agent].RFA << ")" << endl;
            };
        }
        else if (!C4) {
            if (Plot == "On") {
                outputLog << "Level " << LevelCount << ": not cleared since seller hasn't enough quantities to sell ("
                          << Market[Asks[k].Agent].Stocks[Asks[k].Stock].StockQuantity << ")" << endl;
            };
        };
    ...
    if (Plot == "On") {
        outputLog << endl << "TempClearingPriceLast=" << Res[0] << ", TotalStockQuantityTradedLast=" << Res[1]
                  << ", CountLast=" << Res[2] << endl;
    };
    outputLog.close();
...
int
StockOrderBook::MetaorderInjection(vector<Agent> &Market, int SharesOutstanding, int MetaorderImpact, int OBLevelSize,
    ...
    if (OBLevelSize > -1) {
        ...
        if ((BidOrAskMetaorder == 0) && (OBLevelSize > -1)) {
            ...
            Market[Bids[MetaorderLevel].Agent].RFA += ceil(
                    Bids[MetaorderLevel].Q * (Bids[MetaorderLevel].Price + Asks[MetaorderLevel].Price) /
                    2); // This agent is made richer in order to buy
            Res = MetaorderLevel;
            outputMetalog << "Bid metaorder injected at OB level " << MetaorderLevel << ": agent "
                          << Bids[MetaorderLevel].Agent << " buys " << Bids[MetaorderLevel].Q << " for £"
                          << Bids[MetaorderLevel].Price << endl;
            MetaorderLastAgent = Bids[MetaorderLevel].Agent; // Agent placing the order
            MetaorderNature = 0; // 0 for Bid and 1 for Ask
            Credit = ceil(Bids[MetaorderLevel].Q * (Bids[MetaorderLevel].Price + Asks[MetaorderLevel].Price) /
                          2); // Extra RFA (for a buy) or stock quantity (for a sell) temporarily credited to the agent
        } else if ((BidOrAskMetaorder == 1) && (OBLevelSize > -1)) {
            Asks[MetaorderLevel].Q = int(MetaorderImpact * SharesOutstanding / 100);
            Market[Asks[MetaorderLevel].Agent].Stocks[0].StockQuantity +=
                    Asks[MetaorderLevel].Q + 1; // This agent is made richer in order to sell
            Res = MetaorderLevel;
            outputMetalog << "Ask metaorder injected at OB level " << MetaorderLevel << ": agent "
                          << Asks[MetaorderLevel].Agent << " sells " << Asks[MetaorderLevel].Q << " for £"
                          << Asks[MetaorderLevel].Price << endl;
            MetaorderLastAgent = Asks[MetaorderLevel].Agent; // Agent placing the order
            MetaorderNature = 1; // 0 for Bid and 1 for Ask
            Credit = double(Asks[MetaorderLevel].Q +
                            1); // Extra RFA (for a buy) or stock quantity (for a sell) temporarily credited to the agent
        };
    };
    outputMetalog.close();
    return Res;
