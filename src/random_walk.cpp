...
vector<double> RandomWalk(double InitialValue, double Drift, double Volatility, int TimeSteps, double Seed) {
    ...
    for (int t = 1; t < TimeSteps; t++) {
        ...
        S.push_back(ceil((S[t - 1]) * (1 + Drift * dt + Volatility * (StandardNormal[t]) * sqrt(dt) +
                                       Condition * (Jumps[t]) / 100)));
        Condition = 0;
...
vector<double> CointegratedWalk(vector<double> Master, double Leash, double LeashVolatility, double Accuracy) {
    double Val;
    vector<double> S, L;
    vector<double> StandardNormal = STLRandom((int(Master.size())),
                                              "StandardNormal2"); // Time series of size Master.size() made of values gsl_ran_gaussian (r, 1)
    ...
    for (int t = 1; t < (int(Master.size())); t++) {
        L.push_back(
                (L[t - 1]) * (1 + LeashVolatility * (gsl_ran_gaussian(r, 1)))); // The leash is elastic with its own vol
        Val = S[t - 1] + LeashVolatility * (S[t - 1]) * (StandardNormal[t]);
        // Asset comes back if too far
        if ((Master[t] - Val) > ((L[t]) * (Master[t]))) {
            Val = S[t - 1] + LeashVolatility * (S[t - 1]) * (abs(StandardNormal[t]));
        };
        if ((Val - Master[t]) > ((L[t]) * (Master[t]))) {
            Val = S[t - 1] + LeashVolatility * (S[t - 1]) * (-abs(StandardNormal[t]));
        };
        if ((gsl_ran_poisson(r, 1)) * (gsl_ran_poisson(r, 1)) >= Accuracy) {
            Val = Master[t] * (1 + 0.10 * StandardNormal[t]);
        }; //This poisson event is computed as happening 0.2, 0.75, 1, 2 per year for Accuracy=13,11,10,9 on average and brings back the cointegrated walk to the Master time series (after a news for example that reveals TruePresentValue)
        S.push_back(Val);
    };
    return S;
}