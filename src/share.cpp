...
void Share::Gen(const filesystem::path &Name, gsl_matrix *Macroeconomics, int FirstDate) {
    ...
    for (int i = 0; i < Size; i++) {
        gsl_matrix_set(Data, 0, i + PastLag, gsl_matrix_get(Macroeconomics, 0, i));
    }; //Dates
    for (int i = 0; i < Size; i++) {
        for (int k = 0; k < VectorSize; k++) {
            if ((LocalDates[k] == gsl_matrix_get(Macroeconomics, 0, i)) && Cond == 0) {
                FirstStep = k;
                Cond = 1;
            };
    ...
    for (int i = 1; i < Size; i++) {
        if (gsl_matrix_get(Data, 1, i + PastLag) < 0.0000001) {
            gsl_matrix_set(Data, 1, i + PastLag, gsl_matrix_get(Data, 1, i + PastLag - 1));
        };
        if (gsl_matrix_get(Data, 2, i + PastLag) < 0.0000001) {
            gsl_matrix_set(Data, 2, i + PastLag, gsl_matrix_get(Data, 2, i + PastLag - 1));
        };
        if (gsl_matrix_get(Data, 3, i + PastLag) < 0.0000001) {
            gsl_matrix_set(Data, 3, i + PastLag, gsl_matrix_get(Data, 3, i + PastLag - 1));
        };
        if (gsl_matrix_get(Data, 4, i + PastLag) < 0.0000001) {
            gsl_matrix_set(Data, 4, i + PastLag, gsl_matrix_get(Data, 4, i + PastLag - 1));
        };
        if (gsl_matrix_get(Data, 5, i + PastLag) < 0.0000001) {
            gsl_matrix_set(Data, 5, i + PastLag, gsl_matrix_get(Data, 5, i + PastLag - 1));
        };
    };
...
void Share::ProcessBrokerData(const filesystem::path &output_dir) {
        ...
        while (getline(LineStream, Item, ',')) {
            ...
            if ((LineNb == 2) && (ItemNb == 1)) {
                for (int i = 0; i < int(W[0].size()); i++) {
                    if (((Item.c_str() == W[0][i]) || (Item.c_str() + Suffix == W[0][i])) && (W[2][i] == Country) &&
                        (W[3][i] == "Yes")) {
                        Symbol = W[0][i];
                        Title = W[1][i];
                        InPF = "In";
    ...
    if (InPF == "In") {
        ...
        if (Exchange == "LSE") {
            outputLSE << File << ',' << Symbol << ',' << Title << ',' << Exchange << ',' << Country << ',' << Currency
                      << endl;
        }
        if (Exchange == "NYSE") {
            outputNYSE << File << ',' << Symbol << ',' << Title << ',' << Exchange << ',' << Country << ',' << Currency
                       << endl;
        }
        if (Exchange == "NASDAQ") {
            outputNASDAQ << File << ',' << Symbol << ',' << Title << ',' << Exchange << ',' << Country << ','
                         << Currency << endl;
        }
    };
}